...
def convert_to_markdown(poem):
    try:
        lines = [] 
        lines.append(f"# {poem['title']}·{poem['section']}\n")
        lines.append(f"\n**作者**\n")
        lines.append(f"{poem['author']}\n")
        lines.append(f"**朝代**\n")
        lines.append(f"{poem['dynasty']}\n")
        lines.append(f"**内容**\n")
        for paragraph in poem['content']:
            ...
            with open(output_file, 'w', encoding='utf-8') as f:
                ...
                for poem in poetry_data:
                    try:
                        ...
                        poem['content'] = [cc.convert(p) for p in poem['content']]
                        poem['section'] = cc.convert(poem['section'])
                        poem['dynasty'] = '战国'
...
def main():
    poetry_dir = '/home/wqh/projects/ChinesePoetry/楚辞'
